...
def bookings():
    ...
    if search:
        ...
        bookings_list = Booking.query.join(User).join(Space).filter(
            (User.nama_lengkap.ilike(f'%{search}%')) | 
            (Space.nama_ruangan.ilike(f'%{search}%'))
...
def update_booking_status(booking_id):
    ...
    if new_status in ['pending', 'sukses', 'batal']:
        booking.status = new_status
        db.session.commit()
        flash(f'Status pesanan #{booking.id} berhasil diubah menjadi {new_status.upper()}.', 'success')
    else:
...
# ==================== KELOLA RUANGAN (SPACE) ====================
@admin_bp.route('/spaces')
@login_required
def spaces():
    """Menampilkan semua daftar ruangan."""
    search = request.args.get('search', '')
    
    if search:
        spaces_list = Space.query.filter(
            Space.nama_ruangan.ilike(f'%{search}%')
        ).order_by(Space.id.asc()).all()
    else:
        spaces_list = Space.query.order_by(Space.id.asc()).all()
    
    categories = Category.query.all()
    
    return render_template('admin/spaces.html', title='Kelola Ruangan', 
                           spaces=spaces_list, categories=categories, search=search)
...
def delete_space(space_id):
    ...
    if space.bookings:
        flash(f'Tidak bisa menghapus "{space.nama_ruangan}" karena masih ada {len(space.bookings)} pesanan terkait.', 'danger')
        return redirect(url_for('admin.spaces'))
